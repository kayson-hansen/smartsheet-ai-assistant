...
# Initialize client. Uses the API token in the variable api_key
...
result = smart.Sheets.import_xlsx_sheet(
    _dir + '/Sample Sheet.xlsx', header_row_index=0)
...
if rowsToUpdate:
    print("Writing " + str(len(rowsToUpdate)) +
          " rows back to sheet id " + str(sheet.id))
